...
The SDK automatically adjusts its TypeScript configuration to match your project's settings during installation. If your project's `tsconfig.json` has `moduleResolution` set to `"node"`, the SDK will automatically update its own configuration to use the same setting.

This ensures compatibility between your project's TypeScript configuration and the SDK's internal TypeScript setup.

You can also manually run the configuration adjustment script:

```sh
npm run adjust-tsconfig
# or
yarn adjust-tsconfig
```
...
The Klaviyo React Native SDK supports both `"moduleResolution": "Bundler"` and `"moduleResolution": "node16"` TypeScript configurations. This ensures compatibility with different project setups and eliminates the need for custom type declaration workarounds.

### How It Works

The SDK automatically provides appropriate type declarations based on your project's module resolution strategy:

1. **For projects using `"moduleResolution": "Bundler"`**: The SDK uses the default type declarations optimized for modern bundlers
2. **For projects using `"moduleResolution": "node"`**: The SDK automatically adjusts to use `"moduleResolution": "node16"` which is compatible with modern TypeScript features like `customConditions`

### Benefits

- **Dual Support**: Works seamlessly with both module resolution strategies
- **Automatic Selection**: TypeScript automatically uses the appropriate type declarations
- **Backward Compatibility**: Maintains compatibility with existing projects
- **No Workarounds**: Eliminates the need for custom `klaviyo.d.ts` files

### What This Means for You

If your project uses `"moduleResolution": "node"`:

1. You can keep your existing TypeScript configuration unchanged
2. The SDK will automatically adjust to use `"moduleResolution": "node16"` for better compatibility
3. No need to create custom type declaration files or workarounds
4. All TypeScript features (IntelliSense, type checking, `customConditions`, etc.) work as expected

This change ensures that the SDK works correctly regardless of your project's TypeScript module resolution strategy, providing a seamless development experience.
